...
inline bool __cargs_data_unready(const uint32_t __arg_index)
{
    return
        !_cargs_data_args 
        || __arg_index >= _cargs_data_packs.size
        || _cargs_data_packs.packages[__arg_index].count == 0;
}
...
void _cargs_update_data_buffer(const uint32_t _arg_index)
{
    ...
    const uint32_t _cargs_old_count = _cargs_data_packs.packages[_arg_index].count;
    //Empty brackets to dellocate stack memory
    {
        _cargs_anonymous_node* _cargs_ptr = _cargs_redundant_opt_data[_arg_index].first_node;
        while(_cargs_ptr)
        {
            _cargs_data_packs.packages[_arg_index].count += _cargs_ptr->package.count;
            _cargs_ptr = _cargs_ptr->next;
        }

        char** _cargs_old_value_pointers = _cargs_data_packs.packages[_arg_index].values;
        _cargs_data_packs.packages[_arg_index].values = (char**)malloc(sizeof(char*) * _cargs_data_packs.packages[_arg_index].count);
        memcpy(_cargs_data_packs.packages[_arg_index].values, _cargs_old_value_pointers, sizeof(char*) * _cargs_old_count);
    }

    for(uint32_t copy_offset_point = _cargs_old_count; _cargs_redundant_opt_data[_arg_index].first_node;)
    {
        memcpy(
            _cargs_data_packs.packages[_arg_index].values + copy_offset_point, 
...
inline uint32_t cargs_get_data_count(const uint32_t __arg_index)
{
    ...
    return _cargs_data_packs.packages[__arg_index].count;
}
...
inline char** cargs_get_data(const uint32_t __arg_index)
{
    ...
    _cargs_update_data_buffer(__arg_index);
    return _cargs_data_packs.packages[__arg_index].values;
}